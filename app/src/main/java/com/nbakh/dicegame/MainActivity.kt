...
class MainActivity : AppCompatActivity() {
    ...
    fun gameManager() {
        ...
        if(isFirstThrow){ //on First throw
            if(sum==7 || sum==11){
                youWon()
                resulttextview.text = "Везунчик! Ты выиграл"
            }
            else if (sum==2 || sum==3 || sum==12){
                youLoose()
                resulttextview.text = "Неповезло!\nТы проиграл"
            }
            else {
                target = sum
                targettextview.text = "Цель: ${target.toString()}"
                resulttextview.text = "Брось снова"
            }
            isFirstThrow = false
        }
    ...
    fun youWon(){
        resulttextview.text = "Ты выиграл!"
        resulttextview.setTextColor(Color.BLUE)
        tvDesignForWinLoose()
    }
    fun youLoose(){
        resulttextview.text = "Ты проиграл!"
